  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="mx-auto max-w-4xl px-4 py-8">
        ...
        <h2 className="mb-6 text-2xl font-bold text-gray-900">生徒詳細: {sid}</h2>
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_1fr]">
                ...
                {results.map((r) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full rounded-lg border p-3 text-left transition ${isSelected ? "border-blue-500 bg-blue-50" : "bg-white hover:bg-gray-50"}`}
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-sm font-medium text-gray-900">{gradeLabel} {exam?.title ?? r.examId}</p>
                          <p className="text-xs text-gray-500">{new Date(r.completedAt).toLocaleDateString("ja-JP", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}</p>
                        </div>
                        <div className="text-right">
                          ...
                          <p className="text-xs text-gray-500">{r.score}/{r.totalPoints}</p>
                        </div>
                      </div>
                      ...
                      {isSelected && r.sectionScores && (
                        <div className="mt-3 border-t pt-2 space-y-1">
                          <p className="text-xs font-bold text-gray-700">大問別正答率</p>
                          {r.sectionScores.map((ss) => (
                            <div key={ss.name} className="flex items-center justify-between text-xs">
                              <span className="text-gray-600 truncate max-w-[180px]">{ss.name}</span>
                      ...
                      {isSelected && r.lapTimes && r.lapTimes.length > 0 && (
                        <div className="mt-3 border-t pt-2 space-y-1">
                          <p className="text-xs font-bold text-gray-700 flex items-center gap-1"><Clock size={12} /> 通過タイム</p>
                          {r.lapTimes.map((lap) => (
                            <div key={lap.sectionName} className="flex items-center justify-between text-xs">
                              <span className="text-gray-600 truncate max-w-[180px]">{lap.sectionName}</span>
          ...
          <div>
            ...
            <div className="mb-4 rounded-lg border bg-white p-3">
              <p className="mb-2 text-xs font-bold text-gray-700">先生からのコメント</p>
              <textarea
                value={comment}
            ...
            <div className="space-y-2">
              {notes.map((note) => (
                <div key={note.id} className="rounded-lg border bg-white p-3">
                  <p className="text-xs text-gray-400">{new Date(note.createdAt).toLocaleDateString("ja-JP", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}</p>
                  ...
                  <div className="mt-2 rounded bg-blue-50 p-2 text-xs text-gray-700">
                    <p className="font-bold text-blue-800 mb-1">先生のコメント:</p>
                    <p className="whitespace-pre-wrap">{note.teacherComment}</p>
                  </div>
                </div>
              ))}
              {notes.length === 0 && <p className="text-xs text-gray-400">まだカウンセリング記録がありません</p>}
            </div>
          </div>
        </div>
      </main>
