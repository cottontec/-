...
export default function ClassDetailPage() {
  ...
  if (loading) return <div className="min-h-screen bg-gray-50"><Header /><div className="flex items-center justify-center py-32 text-gray-500">読み込み中...</div></div>;
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="mx-auto max-w-3xl px-4 py-8">
        ...
        <section className="mb-8">
          <h3 className="mb-3 flex items-center gap-2 text-lg font-semibold text-gray-900"><Users size={18} /> 生徒一覧</h3>
          {members.length > 0 ? (
            ...
            <p className="text-sm text-gray-500">まだ生徒がいません。招待コードを共有してください。</p>
          )}
        </section>
        ...
        <section>
          <div className="mb-3 flex items-center justify-between">
            <h3 className="flex items-center gap-2 text-lg font-semibold text-gray-900"><BookOpen size={18} /> 課題</h3>
            <button onClick={() => setShowAssign(true)} className="flex items-center gap-1 rounded border px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-50">
              <Plus size={14} /> 課題を追加
            </button>
          </div>

          {showAssign && (
            <form onSubmit={handleAssign} className="mb-4 rounded-lg border bg-white p-4 space-y-3">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">課題タイトル</label>
                <input type="text" value={assignTitle} onChange={(e) => setAssignTitle(e.target.value)} className="w-full rounded-md border px-3 py-2 text-sm" required />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">試験を選択</label>
                <select value={assignExam} onChange={(e) => setAssignExam(e.target.value)} className="w-full rounded-md border px-3 py-2 text-sm" required>
              ...
              <div className="flex gap-2">
                ...
                <button type="button" onClick={() => setShowAssign(false)} className="rounded-md border px-4 py-2 text-sm text-gray-600">キャンセル</button>
              </div>
            </form>
          )}
            ...
            <div className="rounded-lg border bg-white divide-y">
              {assignments.map((a) => (
                <div key={a.id} className="px-4 py-3">
                  ...
                  <p className="text-xs text-gray-500">試験: {a.examId}</p>
                </div>
              ))}
            </div>
          ) : (
