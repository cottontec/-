  ...
  return (
    <div className="min-h-screen bg-[var(--background)]">
      ...
      <main className="mx-auto max-w-3xl px-4 py-8">
        <Link href={exam ? `/grade/${exam.grade}` : "/"} className="mb-4 inline-flex items-center gap-1 text-sm text-[var(--muted)] hover:text-[var(--foreground)]">
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            {explanation.sections.map((sec) => (
              <div key={sec.name} className="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-6">
                <div className="mb-3 flex items-center justify-between">
                  <h3 className="text-lg font-semibold text-[var(--foreground)]">{sec.name}</h3>
                  <span className="rounded bg-[var(--surface-2)] px-2 py-0.5 text-xs text-[var(--muted)]">{sec.questionRange}</span>
                </div>
          ...
          <div className="rounded-lg border border-[var(--border)] bg-[var(--surface)] py-16 text-center">
            <FileQuestion size={48} className="mx-auto text-[var(--muted)]" />
            <p className="mt-4 text-lg font-medium text-[var(--muted)]">解説は準備中です</p>
            <p className="mt-1 text-sm text-[var(--muted)]">先生が解説コンテンツを追加すると、ここに表示されます</p>
          </div>
        )}
      </main>
    </div>
